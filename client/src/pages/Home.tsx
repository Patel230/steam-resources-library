/* Signal Atlas reminder: make a large archive feel calm, directional, and trustworthy. */
/* Signal Atlas explorer: an editorial field guide with asymmetric routes, quiet mineral surfaces, and provenance-led microcopy. */
/* Signal Atlas explorer: retain the mineral-paper field-guide layout, editorial hierarchy, and visible provenance cues while data arrives progressively. */
...
import { CatalogRow, accessLabel, catalogCountries, displayNotes, displaySourceTitle, displayTitle, hasAccessCaveat, initialCatalog, isFreeResource, lastVerifiedDate, lazyCatalogChunkNames, lazyChunksForCountries, loadCatalogChunks, resourceKind, resourceType, resourceTypeOptions, rowTracks, sourceQuality, sourceQualityOptions, trackDefinitions, trackKeys, verifiedWithinDays, verificationLabel } from "@/lib/catalog";
...
function downloadFilteredCsv(rows: CatalogRow[]) {
  ...
  anchor.href = url;
  anchor.download = `signal-atlas-filtered-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return (
    <div className="site-shell">
      <header className="site-header">
        <a className="brand-lockup" href="#top" aria-label="Signal Atlas home">
      ...
      <main id="top">
        <section className="hero-section">
          ...
          <div className="hero-inner">
            <div className="hero-copy">
              ...
              <p className="hero-description">A living field guide to General Aptitude, Engineering Mathematics, and Discrete Mathematics resources from exam bodies, universities, and contest organizers around the world.</p>
            ...
            <div className="hero-note" aria-label="Archive note">
              ...
              <div className="hero-note__footer"><span>Signal Atlas</span><span>14.08.26</span></div>
            </div>
          </div>
        ...
        <section id="subjects" className="section section-subjects">
          <div className="section-heading section-heading--split">
            <div><p className="eyebrow"><span className="eyebrow-marker coordinate-marker" /> The subject system</p><h2>Three routes into<br /><em>the archive.</em></h2></div>
            <p className="section-intro">The catalog groups resources by the kind of thinking they train. Move between tracks, then narrow by country, exam family, or format.</p>
          </div>
          <div className="subject-grid">
            {trackKeys.map((track) => {
              ...
              return <button key={track} className={`subject-card subject-card--${track.toLowerCase()}`} onClick={() => { updateFilter("track", track); document.getElementById("explore")?.scrollIntoView({ behavior: "smooth" }); }}>
                <div className="subject-card__image"><img src={definition.image} alt="" /></div>
          ...
          <div className="coverage-panel" aria-label="Free-resource country coverage status">
            <div className="coverage-panel__intro"><div><p className="eyebrow eyebrow--muted">Coverage ledger</p><h3>{coverageStats.freeMemberStates} of 193 member states have free-tranche records.</h3><p>Signal Atlas keeps pending countries visible instead of filling gaps with invented entries.</p></div><span className="coverage-panel__percent">{coverageStats.progress}%</span></div>
          ...
          <div className="explorer-toolbar-mobile"><Button variant="outline" className="filter-toggle" onClick={() => setIsFilterOpen((open) => !open)}><SlidersHorizontal size={17} /> Filters {activeFilterCount > 0 && <span className="filter-count">{activeFilterCount}</span>}</Button><span>{prettyCount(filteredRows.length)} results</span></div>
          <div className="explorer-layout">
            <aside className={`filter-rail ${isFilterOpen ? "filter-rail--open" : ""}`}>
              ...
              <div className="filter-rail__route" aria-label="Subject route legend"><span className="filter-rail__route-line" /><span className="rail-route rail-route--ga">GA</span><span className="rail-route rail-route--em">EM</span><span className="rail-route rail-route--dm">DM</span></div>
              <div className="filter-group"><label>Subject / route</label><div className="segmented-control">{["All", ...trackKeys].map((value) => <button key={value} className={`${filters.track === value ? "is-active" : ""} ${value !== "All" ? `track-filter--${value.toLowerCase()}` : ""}`} onClick={() => updateFilter("track", value as FilterState["track"])}>{value}</button>)}</div></div>
        ...
        <section className="print-export-sheet" aria-labelledby="print-export-title">
          <div className="print-export-sheet__header"><p className="eyebrow">Signal Atlas / filtered export</p><h2 id="print-export-title">{prettyCount(filteredRows.length)} matching records</h2><p>Generated from the current explorer filters on {new Intl.DateTimeFormat("en", { dateStyle: "long" }).format(new Date())}.</p></div>
        ...
        <section id="about" className="about-band"><div className="about-band__mark"><Bookmark size={22} /></div><div><p className="eyebrow eyebrow--light"><span className="eyebrow-marker coordinate-marker" /> Why this archive exists</p><h2>More practice is easy.<br /><em>Better practice is the work.</em></h2></div><p>Signal Atlas is a navigational layer over a scattered world of official exam archives, contest problem sets, university assignments, and solution collections. It reports the record honestly, keeps provenance visible, and gives you a clear next move.</p></section>
      </main>

      <footer className="site-footer"><a className="brand-lockup" href="#top"><img src="/manus-storage/signal-atlas-mark_db2cb221.png" alt="" className="brand-mark" /><span><strong>Signal</strong> Atlas</span></a><span className="site-footer__routes" aria-label="Signal Atlas subject routes"><span className="site-footer__route site-footer__route--ga"><i /> GA</span><span className="site-footer__route site-footer__route--em"><i /> EM</span><span className="site-footer__route site-footer__route--dm"><i /> DM</span></span><a className="site-footer__coverage-link" href="/coverage">Country directory <ArrowRight size={13} /></a><span>Built for curious learners</span></footer>
