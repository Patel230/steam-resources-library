"""Generate a verified Indonesia tranche from BINUS University's ICPC Jakarta 2020 PDFs.

Every candidate is independently checked for a direct unauthenticated HTTP 200 PDF
response, English contest-problem cues, and absence from the current catalog.
"""
...
OUTPUT = DATA / "indonesia_binus_icpc2020_verified_resources.csv"
AUDIT = ROOT / "research/indonesia_binus_icpc2020_url_audit.csv"
SOURCE_URL = "https://competition.binus.ac.id/icpc2020/"
VERIFY_DATE = "2026-08-15"
TASKS = [(letter, f"https://competition.binus.ac.id/icpc2020/{letter}.pdf") for letter in "ABCDEFGHIJKLM"]
...
def verify(url: str) -> tuple[int, str, str, str]:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        lowered = " ".join(text_result.stdout.split()).lower()
...
def main() -> None:
    ...
    if not verified:
        raise RuntimeError("No eligible BINUS ICPC 2020 records; refusing to write an empty tranche")
    ...
    with OUTPUT.open("w", newline="", encoding="utf-8") as handle:
        ...
        for letter, url in verified:
            writer.writerow({
                ...
                "source_title": "BINUS University — ICPC Asia Jakarta Regional Contest 2020",
                "source_url": SOURCE_URL,
                "resource_title": f"ICPC Asia Jakarta 2020 — Problem {letter} (Official English PDF)",
                "resource_url": url,
                "resource_class": "Programming contest problem paper",
                "language": "English",
                "notes": "Direct organiser-hosted English ICPC Asia Jakarta 2020 problem PDF from BINUS University. Individual access, PDF type, first-page statement cues, and URL uniqueness were checked before promotion.",
    ...
    with AUDIT.open("w", newline="", encoding="utf-8") as handle:
        ...
        for letter, url, status, content_type, evidence, included, diagnostic in outcomes:
            writer.writerow([SOURCE_URL, url, f"ICPC Asia Jakarta 2020 — Problem {letter}", status, content_type, evidence, included, diagnostic])

    print(f"Wrote {len(verified)} verified BINUS ICPC Asia Jakarta 2020 records to {OUTPUT}")
