...
AUDIT = ROOT / 'research/australia_nesa_2015_local_audit.csv'
OUT = DATA / 'australia_nesa_2015_verified_resources.csv'
...
def main():
    ...
    for audit in csv.DictReader(AUDIT.open(newline='', encoding='utf-8')):
        ...
        rows.append({
            ...
            'source_title': 'NSW Education Standards Authority Mathematics HSC exam archive (official)',
            'source_url': audit['page_url'],
            'resource_title': f"NSW NESA {audit['year']} Mathematics — {title_kind}",
            'resource_url': url,
            'resource_class': 'Solution archive' if kind == 'guide' else 'Exam paper',
            'language': 'English',
            'notes': 'Official NSW NESA public English Mathematics HSC assessment document; substantive examination or marking-guideline PDF retained after browser download and local text audit.',
