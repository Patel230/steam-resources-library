"""Generate a rigorously verified Indonesia tranche from BINUS University's ICPC Jakarta 2022 PDFs.

The organiser-owned BINUS contest page is the provenance source. Each candidate letter
is checked independently for an unauthenticated HTTP 200 PDF response, non-duplicate
URL status, and first-page English problem-statement cues before it can be promoted.
"""
...
OUTPUT = DATA / "indonesia_binus_icpc2022_verified_resources.csv"
AUDIT = ROOT / "research/indonesia_binus_icpc2022_url_audit.csv"
SOURCE_URL = "https://competition.binus.ac.id/icpc2022/"
VERIFY_DATE = "2026-08-15"
TASKS = [(letter, f"https://competition.binus.ac.id/icpc2022/{letter}.pdf") for letter in "ABCDEFGHIJKLM"]
...
def verify(url: str) -> tuple[int, str, str, str]:
    """Return HTTP status, content type, English evidence, and diagnostic outcome."""
...
def main() -> None:
    ...
    for letter, url in TASKS:
        title = f"ICPC Asia Jakarta 2022 — Problem {letter} (Official English PDF)"
        if url in existing:
    ...
    if not verified:
        raise RuntimeError("No eligible BINUS ICPC 2022 records; refusing to write an empty tranche")
    ...
    with OUTPUT.open("w", newline="", encoding="utf-8") as handle:
        ...
        for letter, url in verified:
            writer.writerow({
                ...
                "source_title": "BINUS University — ICPC Asia Jakarta Regional Contest 2022",
                "source_url": SOURCE_URL,
                "resource_title": f"ICPC Asia Jakarta 2022 — Problem {letter} (Official English PDF)",
                "resource_url": url,
                "resource_class": "Programming contest problem paper",
                "language": "English",
                "notes": "Direct, organiser-hosted English ICPC Asia Jakarta 2022 problem PDF from BINUS University. Individual access, PDF type, first-page problem-statement cues, and catalog uniqueness were checked before promotion.",
    ...
    with AUDIT.open("w", newline="", encoding="utf-8") as handle:
        ...
        for letter, url, status, content_type, evidence, included, diagnostic in outcomes:
            writer.writerow([SOURCE_URL, url, f"ICPC Asia Jakarta 2022 — Problem {letter}", status, content_type, evidence, included, diagnostic])

    print(f"Wrote {len(verified)} verified BINUS ICPC Asia Jakarta 2022 records to {OUTPUT}")
